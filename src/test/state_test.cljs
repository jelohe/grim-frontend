...
(deftest select-note!-test
  (testing "Marks the selected note"
    (let [initial-notes [{:name "Nota 1" :selected false}
      ...
      (is (true? (-> @state/opened-notes first :selected)
                 "First note should be selected"))
      (is (false? (-> @state/opened-notes second :selected)
                  "Rest of the notes should NOT be selected"))))
  ...
  (testing "Selecting a non-existent note does nothing"
    (let [initial-notes [{:name "Nota 1" :selected false}
      ...
      (is (every? false? (map :selected @state/opened-notes)) 
  ...
  (testing "Selecting a note when another is already selected"
    (let [initial-notes [{:name "Nota 1" :selected true}
      ...
      (is (false? (-> @state/opened-notes first :selected)
                  "Previously selected not should be unselected"))
      (is (true? (-> @state/opened-notes second :selected)
                 "New note should be selected"))))
  )
